...
const login = async (payload) => {
    ...
    if (auth.provider === client_1.LoginProvider.GOOGLE)
    ...
    const result = await prisma_1.default.$transaction(async (tn) => {
        if (payload.verifyAccount) {
            ...
            if (updatedAuth) {
                ...
                const name = updatedAuth.user?.name;
...
const sendOtp = async (email) => {
    ...
    (0, sendEmail_1.sendEmail)(email, subject, path, { otp, name: auth.user?.name });
};
const resetPassword = async (payload) => {
    ...
    const replacements = { name: auth.user?.name };
